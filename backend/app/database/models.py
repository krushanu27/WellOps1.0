...
class Survey(Base):
    ...
    survey_type: Mapped[str] = mapped_column(
        Enum(SurveyType, name="survey_type"),
        nullable=False,
...
class SurveyVersion(Base):
    ...
    questions = relationship(
        "SurveyQuestion", back_populates="version", cascade="all, delete-orphan"
    )
    submissions = relationship(
        "SurveySubmission", back_populates="version", cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "survey_id",
            "version_number",
            name="uq_survey_versions_survey_id_version_number",
        ),
    )
...
class SurveyQuestion(Base):
    ...
    type: Mapped[str] = mapped_column(
        Enum(QuestionType, name="survey_question_type"),
        nullable=False,
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "version_id",
            "question_key",
            name="uq_survey_questions_version_id_question_key",
        ),
        CheckConstraint(
            "display_order >= 0",
            name="ck_survey_questions_display_order_nonnegative",
        ),
    )
...
class SurveySubmission(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "version_id",
            "user_id",
            name="uq_survey_submissions_version_id_user_id",
        ),
    )
...
class SurveyAnswer(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "submission_id",
            "question_id",
            name="uq_survey_answers_submission_id_question_id",
        ),
    )